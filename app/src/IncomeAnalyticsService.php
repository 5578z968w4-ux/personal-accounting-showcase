...
final class IncomeAnalyticsService
{
    ...
    public function search(
        ...
        [$where, $params] = $this->incomeWhere($periodType, $periodFrom, $periodTo, $keyword, $category);
        ...
        $rowsStatement = $this->pdo->prepare(
            'SELECT id, record_date, source_name, amount, account_id, account_name, accounting_month, category
             FROM incomes
             WHERE ' . $where . '
    ...
    private function incomeWhere(
        ...
        if ($keyword !== '') {
            $where[] = '(source_name LIKE :source_keyword OR category LIKE :category_keyword)';
            $params['source_keyword'] = '%' . $keyword . '%';
